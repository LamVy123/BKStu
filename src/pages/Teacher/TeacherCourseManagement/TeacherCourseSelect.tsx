    ...
    useEffect(() => {
        const fetchCourseList = async () => {
            if (auth.userInfor.uid) {
                ...
                const teacherCourseCol = collection(teacherDocRef, 'course')
                let teacherCourseQuery = query(teacherCourseCol)

                if (searchValue == '') {
                    teacherCourseQuery = query(teacherCourseQuery, where('status', '==', 'on_going'))
                } else {
                    teacherCourseQuery = query(teacherCourseQuery, or(where('subject_code', '==', searchValue), where('semester', '==', searchValue)))
                }

                let list: Course[] = [];
                const courseQuerrySnapshot = await getDocs(teacherCourseQuery)
                                        ...
                                        courseList.map((clas) => {
                                            return (
                                                <div key={clas.id} className="col-span-1 w-full h-40 border border-gray-300 border-solid rounded-md flex flex-col hover:cursor-pointer hover:border-black"
                                                    onClick={() => {
                                                        if (clas.id != '') {
                                                            navigate('/course_management/' + clas.id)
                                                        }
                                                    }}>
